...
class AuthController {
  static async getConnect(request, response) {
    ...
    const buffer = Buffer.from(userEmail, 'base64');
    userEmail = buffer.toString('ascii');
    const data = userEmail.split(':'); // this has the password also
    if (data.length !== 2) {
	...
	response.status(200).json({ token });
        ...
        response.sttus(401).json({ error: 'Unauthorized' });
      }
    });
  }
